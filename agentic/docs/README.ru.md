...
Для рискованных и сквозных изменений действует principal-level планка: актуальная документация, понятный след решений, ступени валидации, откат и аккуратная передача контекста до write-heavy реализации.

## Предварительные требования

- Codex, Claude Code или другая агентная среда с поддержкой MCP.
- Настроенный Context7 MCP как `context7`/`mcpcontext7`, чтобы агент получал актуальную документацию по библиотекам, API, CLI, платформам и конфигурациям до генерации кода.
- Рекомендуется проектная память, например Obsidian, для заметок и связей графа.
